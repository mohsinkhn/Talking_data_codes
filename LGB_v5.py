...
handler = logging.FileHandler('LGB_v5.log')
...
if __name__ == "__main__":
    ...
    model, _ = run_lgb(tr, val, LGB_PARAMS, logger, feats=base_feats, is_develop=True, save_preds=False)
    test_preds = model.predict_proba(test[base_feats])[:, 1]
    prepare_submission(test_preds, save_path="basefeats_test_preds.csv")
    ...
    for col in ['app', 'channel', 'os', 'device', 'ip', 'ip_device_os', 'ip_device_os_app', 'ip_device_os_app_channel']:
        ...
        tr[count_col], val[count_col] = get_count_feature(
                                         tr, val, [col], 'is_attributed', 
                                         tr_filename=os.path.join("../output", "tr_{}_count_{}.pkl".format(col, SEED)), 
                                         val_filename=os.path.join("../output", "val_{}_count_{}.pkl".format(col, SEED)),
                                         seed=SEED, rewrite=True )
        
        feats2.append(count_col)
        ...
        if (score > best_score):
            ...
            train[count_col], test[count_col] = get_count_feature(
                                         train, test, [col], 'is_attributed', 
                                         tr_filename=os.path.join("../output", "train_{}_count_{}.pkl".format(col, SEED)), 
                                         val_filename=os.path.join("../output", "test_{}_count_{}.pkl".format(col, SEED)),
                                         seed=SEED, rewrite=True )
            ...
            prepare_submission(test_preds, save_path="feats{}_test_preds.csv".format(len(feats)))
        else:
            feats2.remove(count_col)
            
        logger.info("Gnerating feature: {} for tr/val set".format(mean_col))
        tr[mean_col], val[mean_col] = get_mean_feature(tr, val, [col], 'is_attributed',   
                                         tr_filename=os.path.join("../output", "tr_{}_cv{}mean_{}.pkl".format(col, CV_FOLDS, SEED)), 
                                         val_filename=os.path.join("../output", "val_{}_cv{}mean_{}.pkl".format(col, CV_FOLDS, SEED)),
                                         cv=CV_FOLDS, seed=SEED, rewrite=True )
        feats2.append(mean_col)
        feats = base_feats + feats2
        ...
        if (score > best_score):
            best_score = score
            logger.info("Gnerating feature: {} for train/test set".format(mean_col))
            train[mean_col], test[mean_col] = get_mean_feature(train, test, [col], 'is_attributed', 
                                         tr_filename=os.path.join("../output", "train_{}_cv{}mean_{}.pkl".format(col, CV_FOLDS, SEED)), 
                                         val_filename=os.path.join("../output", "test_{}_cv{}mean_{}.pkl".format(col, CV_FOLDS, SEED)),
                                         cv=CV_FOLDS, seed=SEED, rewrite=True )
            ...
            feats2.remove(mean_col)
        
    logger.info("Saving train and test features")  
    train[feats2].to_csv("../output/train_featsset2.csv", index=False)
    
    test[feats2].to_csv("../output/test_featsset2.csv", index=False)
