...
def process_file(filename, new_filename, dict_cols, group_dicts, chunksize=10**6):
    ...
    base_feats = ['ip', 'app', 'device', 'os', 'channel', 'is_attributed','click_time' ,'dayofweek', 'hourofday',  'ip_device_os',
    ...
    for i, chunk in enumerate(pd.read_csv(filename, dtype=dtypes, chunksize=chunksize)):
        ...
        for cols, di in zip(dict_cols, group_dicts):
            ...
            chunk[col_name] = chunk['tmp'].map(di)
            del chunk['tmp']
...
if __name__ == "__main__":
    ...
    process_file("../input/train.csv", "../input/train_base.csv", all_cols, all_dicts, chunksize=10**5)
    process_file("../input/test.csv", "../input/test_base.csv", all_cols, all_dicts, chunksize=10**5)
    process_file("../input/test_supplement.csv", "../input/test_supplement_base.csv", all_cols, all_dicts, chunksize=10**5)
