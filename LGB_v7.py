...
if __name__ == "__main__":
    ...
    logger.info("Reading expanding mean features")
    train_f3 = pd.read_csv("../output/train_featsset3.csv") ###
    test_f3 = pd.read_csv("../output/test_featsset3.csv") ###
    MEAN_COLS = list(train_f3.columns)
    
    logger.info("Merge expanding mean features with base dataframe")
    train = pd.concat([train, train_f3], axis=1) ###
    test  = pd.concat([test, test_f3], axis=1) ###
