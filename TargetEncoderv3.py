...
class TargetEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        #XX should be a daskdataframe
        if isinstance(self.func, str):
            if hasattr(dd.Series, self.func):
                #print("here")
    ...
    def transform(self, X, y=None):
        if isinstance(X, dd.DataFrame):
            ...
            X_transformed = X_transformed.join(self.dictmap.to_frame(), on=self.cols, how='left')[self.cname]

            if self.add_to_orig:
                return dd.concat([X, X_transformed], axis=1, copy=False)
            else:
